"""
构建一个简单的文本cnn模型，基于paddle，静态图。测试
"""
...
def cnn_pd(inputs, output_dim, kernel_size=5, dimension=100, conv_filters=40, stride=2, act='relu',
           words_num=10000, use_bias=True, padding_id=0):
    emb = fluid.embedding(inputs, size=[words_num, dimension], is_sparse=True, padding_idx=padding_id, param_attr="shared_w")
    print('emb', emb.shape)
    emb = fluid.layers.reshape(emb, shape=[emb.shape[0], 1, emb.shape[1], emb.shape[2]])
    print('emb', emb.shape)
    conv_out = fluid.layers.conv2d(emb, num_filters=conv_filters, stride=(stride, 1), filter_size=(kernel_size, dimension), act=act, bias_attr=use_bias)
...
if __name__ == '__main__':
    ...
    pred = cnn_pd(inputs, output_dim=label_num, kernel_size=3, dimension=100, conv_filters=50, stride=1, act='relu',
                  words_num=len(vocabulary), use_bias=True, padding_id=0)
    ...
    for epoch in range(epochs):
        ...
        for x_, y_, step, total_step in data:
            avg_cost_np, tem_loss, accuracy = exe.run(main_program, feed=feeder.feed(zip(x_, y_)), fetch_list=[ave_loss, loss, acc])
            #print(tem_loss)
    ...
    for x_, y_, step, total_step in test_data:
        avg_cost_np, accuracy = exe.run(test_program, feed=feeder.feed(zip(x_, y_)), fetch_list=[ave_loss, acc], use_program_cache=True)
        ave_cost = (ave_cost * (float(step) - 1.0) + avg_cost_np[0]) / float(step)
        ave_acc = (ave_acc * (float(step) - 1.0) + accuracy[0]) / float(step)
